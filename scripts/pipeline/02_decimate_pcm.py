...
Define a taxa de decimação do subconjunto acústico (Jung et al., 2023) e valida
que a decimação preserva a assinatura de falha BPFI/BPFO.

Entra: os .bin PCM int16 a 51.200 Hz gerados por `01_convert_mat_to_pcm.py`
       (data/processed/pcm_raw/ + manifest.json).
Sai:   - .bin decimados por taxa candidata (data/processed/pcm_decimated/<fs>/),
         com manifest.json por taxa — o manifest é versionado, os .bin não
       - métricas em reports/decimation/metrics.json
       - figuras em reports/decimation/*.png
       - relatório legível em reports/decimation/decimation_report.md
       - uma linha por taxa candidata em experiments/registry.csv, no formato
         da seção 4 do CONVENTIONS.md

Racional da validação
---------------------
Em áudio de rolamento, BPFI/BPFO **não** aparecem como tons puros no espectro do
sinal: cada impacto excita ressonâncias de alta frequência da estrutura, e a
periodicidade da falha aparece como *modulação de amplitude* dessas ressonâncias.
Logo o teste correto não é "o bin de 272 Hz sobreviveu?", e sim:

  1. onde mora a energia que separa falha de normal (banda ressonante);
  2. o **espectro de envelope** dessa banda ainda mostra picos em BPFO≈179 Hz,
     BPFI≈272 Hz e harmônicas depois de decimar;
  3. as features que o classificador vai ver (MFCC) continuam separando as
     classes na taxa reduzida;
  4. o custo embarcado (amostras/janela, FFT, RAM) cai como esperado.

Um ponto entra de graça e decide bastante: 51.200 Hz só admite decimação por
fator **inteiro** para 25.600 / 12.800 / 6.400 / ... Hz. As taxas "redondas"
(8.000 e 16.000 Hz) exigem reamostragem racional L/M (interpolar e depois
decimar), que no STM32 não é atendida por `arm_fir_decimate_f32` — esta exige
fator inteiro. O script mede isso e reporta.

Uso
---
    python scripts/pipeline/02_decimate_pcm.py

    # teste rápido (só 10 s de cada classe), sem sujar o registry
    python scripts/pipeline/02_decimate_pcm.py --seconds 10 --no-write-bin --no-registry

    # auto-teste sem os dados reais: gera sinal sintético com assinatura conhecida
    python scripts/pipeline/02_decimate_pcm.py --synthetic --seconds 12 --no-registry
...
def peak_snr(freqs: np.ndarray, mag: np.ndarray, f_target: float) -> dict:
    ...
    return {
        "presente": bool(snr >= SNR_ACCEPT_DB),
        "snr_db": round(snr, 2),
...
# Etapa C — decimação
# --------------------------------------------------------------------------- #
def design_decimation(fs_in: float, fs_out: float, atten_db: float = STOPBAND_TARGET_DB) -> FirSpec:
    """
...
def resample_clip(x: np.ndarray, fs_in: float, fs_out: float, spec: FirSpec) -> np.ndarray:
...
def mfcc(x: np.ndarray, fs: float) -> np.ndarray:
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--no-write-bin", action="store_true", help="não gravar os .bin decimados")
